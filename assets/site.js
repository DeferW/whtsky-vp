(() => {
  ...
  header.innerHTML = `<div class="container header-inner"><a class="brand" href="${root}" aria-label="На главную"><span class="brand-mark" aria-hidden="true"><img class="logo-dark" src="${root}assets/images/logo-white.png" alt=""><img class="logo-light" src="${root}assets/images/logo-black.png" alt=""></span><span data-name></span><span class="brand-caption">VANILLA PROJECT</span></a><nav aria-label="Основная навигация">${routes.map(([id, path, label]) => `<a href="${root}${path}" ${page === id ? 'aria-current="page"' : ''}>${label}${id === 'wiki' ? '<span class="nav-tag">скоро</span>' : ''}</a>`).join('')}</nav><button class="theme-toggle" type="button" data-theme-toggle aria-label="Светлая тема">☀</button></div>`;
